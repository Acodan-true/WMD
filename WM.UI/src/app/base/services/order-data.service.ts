...
export class OrderDataService {
    ...
    public getOrders(facilityId: number, orderNumber: string, statusIds: number[]): Observable<Order[]> {
        ...
        return this.http.get<Order[]>(`https://localhost:7079/api/orders/${facilityId}`, { params });
    }

    public getOrder(id: number): Observable<Order> {
        return this.http.get<Order>(`https://localhost:7079/api/orders/details/${id}`);
    }

    public createOrder(order: Order): Observable<Order> {
        return this.http.post<Order>(`https://localhost:7079/api/orders`, {
    ...
    public updateOrder(orderId: number, statusId: number, items: OrderItem[]): Observable<Order> {
        return this.http.put<Order>(`https://localhost:7079/api/orders`, {
            orderId: orderId,
            statusId: statusId,
            items: items
        });
